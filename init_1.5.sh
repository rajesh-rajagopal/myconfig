...
if [[ -f "${ENVFILE}" ]]; then
  GULD_HOME_DIR=$(cat "${ENVFILE}")
  echo "Overriding home directory: ${GULPHOMEDIR}"
fi
...
### Welcome to the Gulpd configuration file.
  ...
  [meta]
    user = "root"
    vertice_api = "http://192.168.1.15:9000/v2"		# Replace with Vertice Gateway Running IPs
    nsqd = ["192.168.1.15:4150"]			# Replace with Vertice NSQD Running IPs
  ...
  [gulpd]
    ...
    gru_tarball = "https://github.com/megamsys/gru/archive/0.1.tar.gz"
...
case "$OS1" in
   "CentOS")
        sudo service verticegulpd start
